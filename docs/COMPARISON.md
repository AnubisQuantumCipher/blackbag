...
| **Breach check: protocol and what leaves the machine** | HIBP k-anonymity, opt-in per run (`--online`; exit 2 otherwise). Agent hands the CLI one entry per `password` / `passphrase` / `pin` field (record id, title, field name, 5-hex-character SHA-1 prefix); the CLI deduplicates the prefixes, then pads them with random decoys to a multiple of 8 and shuffles (`breach::padded_prefixes`), so the request count does not report how many distinct passwords the vault holds; CLI fetches each bucket with `curl -q --max-time 20 --proto =https --noproxy '*'`, `Add-Padding: true`, `User-Agent: black-bag/<version>`; padding entries (count 0) dropped, an empty body treated as a failed fetch; a failed fetch keeps the previous verdict rather than clearing it, and changing a field drops its verdict at once; agent matches the full hash it never disclosed and folds hits into hygiene as `EXPOSED` (severity High) until lock. Under its systemd unit the agent has no network family at all; an agent started by hand is not so restricted (`crates/blackbag-core/src/breach.rs`; `crates/blackbag-cli/src/main.rs`, `cmd_breach`; `hygiene.rs`, `Issue::Exposed`). Service sees: IP, time, and a request count rounded up to a multiple of 8 | HIBP k-anonymity, opt-in, behind a networking build flag (report §1.5) | HIBP k-anonymity, 5-character SHA-1 prefix, compared locally (report §1.5) | HIBP (report §1.5; protocol detail not given) | Server-side lookup; custom addresses shared with third parties (report §1.5, §3.4) | `pass audit` / `gopass audit` (report §1.5; protocol not researched) |
...
**A small mandatory secret path.** Two dependencies were removed from it
rather than trusted. `totp-rs` took the 2FA shared secret as an ordinary
`Vec<u8>` and held it unwiped for the life of its object, so every code the
deck displayed copied the long-lived credential into heap that was freed
dirty; `blackbag_core::totp` computes HOTP in forty lines with the secret
borrowed straight out of the arena, and is checked against every RFC 6238
Appendix B vector for all three hashes and every RFC 4226 Appendix D vector —
more validation than the crate carried. `aead::Aead::encrypt`, whose blanket
implementation stages the whole plaintext in a fresh heap `Vec`, is gone from
`crypto::seal` for the same reason, and the `Aead` import with it. The D-Bus
client and the HTTP call are likewise hand-written and `curl`-shelled rather
than linked: the agent that holds the key has no network address family at
all. Against the field, KeePassXC's own memory CVEs (2024-33900/33901) come
from Qt containers it does not control, and every cloud client in the matrix
links a full HTTP stack into the process holding the vault key.
